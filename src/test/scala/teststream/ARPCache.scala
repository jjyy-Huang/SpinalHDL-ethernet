...
case class ARPCacheInterface(config: ArpCacheGenerics)
    extends Bundle
    with IMasterSlave {
...
case class ARPCacheReplyInterface(config: ArpCacheGenerics)
    extends Bundle
    with IMasterSlave {
...
class ARPCache(config: ArpCacheGenerics) extends Component {
  ...
  val arpCache = new Mem(
    Bits(
      config.IP_ADDR_WIDTH + config.MAC_ADDR_WIDTH - config.CACHE_INDEX_WIDTH bits
    ),
    config.CACHE_DEPTH
  )
